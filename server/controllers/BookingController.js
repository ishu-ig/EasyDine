...
async function verifyOrder(req, res) {
    try {
        const booking = await Booking.findOne({ _id: req.body.checkid });
        if (!booking) {
        ...
        if (generated_signature !== req.body.razorpay_signature) {
            // ❌ Signature mismatch — delete the pending booking
            await Booking.findByIdAndDelete(req.body.checkid);
            return res.status(400).json({ result: "Fail", message: "Payment verification failed! Booking cancelled." });
        }
        ...
        booking.bookingStatus  = true;   // confirm only after payment
        await booking.save();
...
async function createRecord(req, res) {
    try {
        ...
        let data = new Booking({
            ...req.body,
            // ❌ Don't confirm Net Banking bookings until payment verified
            bookingStatus: isNetBanking ? false : req.body.bookingStatus,
        ...
        // ✅ Send confirmation email only for COD (Net Banking confirmed after verifyOrder)
        if (finalData.bookingStatus) {
            mailer.sendMail({
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9f9f9; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0,0,0,0.1);">
                        ...
                        <div style="background-color: #fff; padding: 15px; border-radius: 8px; margin-top: 15px; box-shadow: 0px 0px 5px rgba(0,0,0,0.1);">
                            <p style="font-size: 16px; margin: 8px 0;"><strong>Restaurant:</strong> ${finalData.resturent.name}</p>
    ...
    try {
        ...
        if (data) {
            ...
            let finalData = await Booking.findOne({ _id: data._id })
                .populate("user", ["name", "username", "email", "phone", "address", "state", "city", "pin"])
                .populate("resturent", ["name", "finalPrice", "address", "phone"]);
            if (finalData.bookingStatus === "false") {
                mailer.sendMail({
                    from: process.env.MAIL_SENDER,
                    to: finalData.user.email,
