...
const S = {
  ...
  heroGradient: {
    ...
    inset: 0,
    background:
      "linear-gradient(to top, rgba(10,5,3,0.72) 0%, rgba(10,5,3,0.1) 55%, transparent 100%)",
    pointerEvents: "none",
  },
  heroMeta: {
    position: "absolute",
  ...
  heroBadgeRow: { display: "flex", flexWrap: "wrap", gap: 8 },
  statusBadge: (open) => ({
    display: "inline-flex",
    alignItems: "center",
    gap: 6,
  ...
  chipRow: { display: "flex", flexWrap: "wrap", gap: 10, marginTop: 14 },
  chip: {
    display: "flex",
    alignItems: "center",
    gap: 8,
  ...
  addSlotRow: { display: "flex", gap: 8, alignItems: "center" },
  addSlotBtn: {
    padding: "10px 18px",
    borderRadius: C.radiusSm,
    border: "none",
    ...
    <div style={{ display: "flex", gap: 4, marginBottom: 12 }}>
      {[1, 2, 3, 4, 5].map((n) => (
        <i
          ...
          style={{
            color: (hovered || value) >= n ? "#F4A044" : "#d1d5db",
            fontSize: "1.2rem",
            cursor: "pointer",
...
export default function ResturentDetailPage() {
  ...
  const [commentSuccess, setSuccess] = useState("");
  ...
  useEffect(() => {
    if (ProductStateData.length > 0)
      setRelated(ProductStateData.filter((x) => x.resturent === data.name));
  }, [ProductStateData, data.name]);

  const fetchComments = useCallback(async () => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_SERVER}/api/resturent/${_id}/review`,
      );
      const json = await res.json();

      console.log("Review API:", json); // debug

      if (json.result === "Done") {
        setComments(json.reviews); // ✅ FIXED
      }
    } catch (error) {
      console.log(error);
    }
  ...
  function generateTimeSlots(openTime, closeTime) {
    ...
    while (start < end) {
      ...
      slots.push(
        `${fmt(start.getHours())}:${fmt(start.getMinutes())} - ${fmt(next.getHours())}:${fmt(next.getMinutes())}`,
      );
      start = next;
    }
    return slots;
  ...
  async function postData(e) {
    ...
    if (selDate < today) {
      setErrorMessage((prev) => ({
        ...prev,
        reservationDate: "Backdate not allowed",
      }));
      return;
    }
    if (!Array.isArray(data.timeSlots) || data.timeSlots.length === 0) {
      setErrorMessage((prev) => ({
        ...prev,
        timeSlots: "Selecting at least one time slot is mandatory",
      }));
      return;
    }
    ...
    if (data.seatAvailable > 0) {
      if (localStorage.getItem("login")) {
        if (window.confirm("Are you sure you want to make a booking?")) {
          // ✅ Step 1: Create booking (status = pending for Net Banking)
          let bookingRes = await fetch(`${SERVER}/api/booking`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              user: localStorage.getItem("userid"),
              bookingStatus: mode === "COD", // true only for COD
          ...
          if (bookingRes.result === "Done") {
            ...
            if (mode === "COD") {
              // ✅ COD: reduce seats immediately and confirm
              let updRes = await fetch(`${SERVER}/api/resturent/user/${_id}`, {
                method: "PUT",
                headers: {
                  "content-type": "application/json",
                  authorization: localStorage.getItem("token"),
                ...
                body: JSON.stringify({
                  ...data,
                  seatAvailable: data.seatAvailable - qty,
                }),
              });
              updRes = await updRes.json();
              navigate(`/confirmationBooking/${bookingId}`);
            } else {
              // ✅ Net Banking: go to payment — seats reduced only after payment verified
                  ...
                  <div style={S.heroBadgeRow}>
                    ...
                    {data.discount > 0 && (
                      <span
                        style={{
                          ...S.heroPill,
                          background: "rgba(200,64,10,0.75)",
                          border: "none",
                        }}
                      >
                        <i
                ...
                <div style={S.chip}>
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        ...S.chipValue,
                        color: data.seatAvailable > 0 ? "#16a34a" : "#ef4444",
                      }}
                    >
              ...
              <div style={S.bookingCard}>
                <div style={S.bookingHeader}>
                  <div style={S.bookingTitle}>
                    <i
                      className="fa fa-calendar-check me-2"
                      style={{ color: "var(--secondary)" }}
                    ></i>
                    Reserve Your Table
                  </div>
                  <div style={S.bookingSubtitle}>
                    ...
                    >
                      <div style={S.qtyRow}>
                        ...
                        <button
                          ...
                          onClick={() =>
                            setQty((p) =>
                              Math.min(data.seatAvailable || 1, p + 1),
                            )
                          }
                          type="button"
                        >
                      ...
                      <span style={{ fontSize: "0.82rem", color: C.gray }}>
                        Max: {data.seatAvailable > 0 ? data.seatAvailable : 0}{" "}
                        seats
                      </span>
                    </div>
                  </div>
            ...
            <div className="row g-4">
              ...
              <div className="col-lg-7">
                <div style={S.reviewsTitle}>
                  <i
                    className="fa fa-comments me-2"
                    style={{ color: C.primary, fontSize: "1.4rem" }}
                  ></i>
                  Guest Reviews
                  {comments.length > 0 && (
                    <span
              ...
              <div className="col-lg-5">
                <div style={S.reviewFormCard}>
                  ...
                  <div style={{ marginBottom: 4 }}>
                    ...
                    <StarPicker value={commentRating} onChange={setRating} />
                  </div>
