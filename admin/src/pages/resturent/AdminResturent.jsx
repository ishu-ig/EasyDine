...
import { deleteResturent, getResturent } from "../../Redux/ActionCreators/ResturentActionCreators";
...
export default function AdminResturent() {
    ...
    function deleteRecord(_id) {
        if (window.confirm("Are you sure you want to delete this item?")) {
            dispatch(deleteResturent({ _id: _id }));
            getAPIData();
            setFlag(!flag)
        }
    }
    ...
    return (
        <>
            <div className="container-fluid">
                {/* Header */}
                <h5 className="text-center text-light bg-primary p-3">Resturent <Link to="/resturent/create"><i className="fa fa-plus text-light float-end pt-1"></i></Link></h5>
                {/* Table */}
                        ...
                        <tbody>
                            {ResturentStateData.map((item) => (
                                <tr key={item._id}>
                                    ...
                                    <td className={item.status ? 'text-success fw-bold' : 'text-danger fw-bold'}>
                                    ...
                                    <td>
                                        <Link to={`/resturent/update/${item._id}`} className="btn btn-primary text-light btn-sm">
