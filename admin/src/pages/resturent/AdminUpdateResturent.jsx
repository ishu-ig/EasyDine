...
import { updateResturent, getResturent } from '../../Redux/ActionCreators/ResturentActionCreators'

export default function AdminUpdateResturent() {
    // let { id } = useParams()
    ...
    function postSubmit(e) {
        ...
        if (errorItem)
            ...
            let item = ResturentStateData.find(x => x._id !== _id && x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase())  // in case of real backend
            // let item = ResturentStateData.find(x => x._id !== id && x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase())
            if (item) {
                setShow(true)
                setError((old) => {
                    return {
    ...
    useEffect(() => {
        (() => {
            ...
            if (ResturentStateData.length) {
                // let item = ResturentStateData.find(x => x._id === id)
    ...
    return (
        <>
            <div className="container">
                <h5 className="text-center text-light bg-primary p-2">Update Resturent <Link to="/resturent"><i className="fa fa-arrow-left text-light float-end pt-1"></i></Link></h5>
                {/* Form */}
                <div className="card mt-3 shadow-sm p-4">
                    <form onSubmit={postSubmit}>
                        ...
                        <div className="mb-3">
                            ...
                            <textarea name="address" value={data.address} onChange={getInputData} className={`form-control ${show && error.discount ? 'border-danger' : 'border-primary'}`} rows={4}></textarea>
