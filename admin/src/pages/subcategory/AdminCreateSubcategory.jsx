...
export default function AdminCreateSubcategory() {
    ...
    function postSubmit(e) {
        ...
        if (errorItem)
            ...
            let item = SubcategoryStateData.find(x => x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase() && x.maincategory === data.maincategory)
            console.log(item)
            if (item) {
                setShow(true)
                setError((old) => {
                    return {
                        ...old,
                        "name": "Subcategory With This MainCategory Already Exist"
                    }
                })
            }
            else {
