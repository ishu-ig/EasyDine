                            ...
                            <tr>
                                ...
                                <td>
                                    <div className="card p-3 bg-light">
                                        ...
                                        <p className="mb-1"><i className="fa fa-envelope"></i> {data?.user?.email}</p>
                                        <p className="mb-1"><i className="fa fa-phone"></i> {data?.user?.phone}</p>
                                        <p className="mb-1"><i className="fa fa-map-marker"></i> {data?.user?.address}, {data?.user?.city}, {data?.user?.state}, {data?.user?.pin}</p>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>Order Status</th>
                                <td className={`fw-bold ${data?.orderStatus !== "Cancelled" ? "text-success" : "text-danger"}`}>
                                    {data?.orderStatus }
                                    {data?.orderStatus !== "Delivered" && data?.orderStatus !== "Cancelled" ? (
                                        ...
                                        >
                                            <option value="Order Is Placed">Order Is Under Process</option>
                                            <option value="Order Is Placed">Order Is Placed</option>
                                            <option value="Order Is Ready For Delivery">
                            ...
                            <tr>
                                <td colSpan={2}>
                                    {data?.orderStatus !== "Delivered" || data?.paymentStatus === "Pending" ? (
                                        <button className='btn btn-primary w-100 text-light' onClick={updateRecord}>Update</button>
                                    ) : null}
                                </td>
                            </tr>
                        </tbody>
